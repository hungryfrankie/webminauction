    ...
    case 4: // finalise auction (submit to db)
        ...
        if ($ERR != 'ERR_') {
            ...
            if (!isset($_SESSION['SELL_action']) || empty($_SESSION['SELL_action'])) {
                ...
                // bof make offer
        /*        if ($system->SETTINGS['mo_only'] && $system->SETTINGS['bn_only_disable'] == 'y' && $system->SETTINGS['mo_only_percent'] < 100) {
                    $query = "SELECT COUNT(*) as count FROM " . $DBPrefix . "auctions
							WHERE closed = 0 AND suspended = 0 AND user = :user_id";
                    $params = array();
                    $params[] = array(':user_id', $user->user_data['id'], 'int');
                    ...
                    if ($totalaucs > 0) {
                        $query = "SELECT COUNT(*) as count FROM " . $DBPrefix . "auctions
								WHERE closed = 0 AND suspended = 0 AND mo_only = 1 AND user = :user_id";
                        $params = array();
                        $params[] = array(':user_id', $user->user_data['id'], 'int');
                        $db->query($query, $params);
                        $totalbnaucs = $db->result('count');
                        $percent = ($totalbnaucs * 100) / $totalaucs;
                        if ($user->user_data['mo_only'] && $system->SETTINGS['mo_only_percent'] <= $percent) {
                            $query = "UPDATE " . $DBPrefix . "users SET mo_only = 0 WHERE id = :user_id";
                            $params = array();
                            $params[] = array(':user_id', $user->user_data['id'], 'int');
                            $db->query($query, $params);
                        }
                        if ($user->user_data['mo_only'] == 0 && $system->SETTINGS['mo_only_percent'] > $percent) {
                            $query = "UPDATE " . $DBPrefix . "users SET bn_only = 1 WHERE id = :user_id";
                            $params = array();
                            $params[] = array(':user_id', $user->user_data['id'], 'int');
                            $db->query($query, $params);
...
// end added for item condition mod
                    'B_SUBTITLE' => ($system->SETTINGS['subtitle'] == 'y')
                    ));
            break;
        }
        // bof item condition  sizes and make offer  here we get the current values in the db for all three 
    ...
    case 1:  // enter auction details
        ...
        foreach ($system->SETTINGS['auction_types'] as $key => $val) {
         if (($key!=3)||($user->permissions['has_store']==true))   $TPL_auction_type .= "\t" . '<option value="' . $key . '" ' . (($key == $atype) ? 'selected="true"' : '') . '>' . $val . '</option>' . "\n";
        }
        ...
        $feevarsset = array();
        $fee_javascript = '';
        $relist_fee = $subtitle_fee = $fee_rp = $fee_bn = $fee_min_bid = $offer_fee= 0;
        $offer_fee= 0;
